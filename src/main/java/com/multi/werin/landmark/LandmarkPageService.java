...
public class LandmarkPageService {
	...
	public void api2() {
        ...
        int apiid = 0;
        for (int i = 0; i < 10; i++) { // 가져온 contentid를 이용한 반복문으로 overview 등을 불러와서 db에 집어넣는 메서드
	...
	public void page(String landmarkName, Model model) {
		//db처리하기 전 전처리/후처리 
		//db처리 요청 
		List<LandmarkApiVO2> list2 = pagedao.page(landmarkName);
		System.out.println(list2.size());
		//views/list.jsp까지 넘어가야 함.==>Model 
		//model을 이용해서 검색결과인 list를 jejupage.jsp까지 넘기자
		model.addAttribute("list2", list2);
