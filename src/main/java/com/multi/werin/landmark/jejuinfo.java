...
public class jejuinfo {
    ...
    static String apiKey = "gRjisabrZIfabIjre8qnZIy4ig724PgKzbxWE5KdbOHL4SD%2FneKsnNw2phKHIowgD5NXuBRaTpMEvSAY2uTA1Q%3D%3D";

    public static void main(String[] args) {
        try {
            int numOfRows = 10;
            int pageNo = 1;
            String info = "detailCommon1";
            String index = "&contentTypeId=12&contentId=2858467&MobileOS=ETC&MobileApp=AppTest&defaultYN=Y&firstImageYN=Y&areacodeYN=Y&catcodeYN=Y&addrinfoYN=Y&mapinfoYN=Y&overviewYN=Y";
            ...
            // 응답이 없거나 빈 데이터일 경우 중단
            if (jsonContent.isEmpty()) {
                System.out.println("API 응답이 없거나 빈 데이터입니다.");
                return;
            }

            // JSON 데이터를 파싱하여 출력
            printJsonData(jsonContent);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
 ...
 //       ObjectMapper objectMapper = new ObjectMapper();
  //      JsonNode rootNode = objectMapper.readTree(json);
    ...
    //    String prettyJson = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(rootNode);
        ...
        // 이 부분에서 필요한 JSON 데이터를 추출하여 사용하면 됩니다.
        // 예를 들어, 특정 필드를 출력하는 경우:
  //      String resultCode = rootNode.path("response").path("header").path("resultCode").asText();
    //    System.out.println("Result Code: " + resultCode);

        // 가독성 좋은 JSON 출력
     //   System.out.println("Pretty JSON Response: " + prettyJson);
    }
}